...
export class BridgeManager {
  ...
  constructor({ engine, hub }) {
    ...
    // 初始化媒体本地路径白名单（对标 OpenClaw mediaLocalRoots）
    const roots = [engine.hanakoHome];
    const prefs = engine.getPreferences();
    if (prefs.desk?.workspace) roots.push(prefs.desk.workspace);
    // 包含 os.tmpdir()，agent tool 可能生成临时文件
