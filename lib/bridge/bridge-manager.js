...
import { downloadMedia, bufferToBase64, detectMime, splitMediaFromOutput, formatSize } from "./media-utils.js";
...
class StreamCleaner {
  ...
  _processLine(line) {
    ...
    if (mediaMatch) {
      try {
        const u = new URL(mediaMatch[1]);
        if (u.protocol === "http:" || u.protocol === "https:") {
          this.extractedMedia.push(mediaMatch[1]);
        }
      } catch { /* 无效 URL */ }
      return null; // 从输出中移除
    }
    ...
    try {
      ...
      if (reply && entry?.adapter) {
        ...
        for (const url of mediaUrls) {
          try { await entry.adapter.sendMedia?.(chatId, url); }
    ...
    try {
      ...
      if (reply && adapter) {
        ...
        for (const url of allMediaUrls) {
          try { await adapter.sendMedia?.(chatId, url); }
