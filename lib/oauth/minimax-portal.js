...
export const minimaxOAuthProvider = {
  ...
  /**
   * 用 auth.json 里的 resourceUrl 替换 baseUrl 的域名部分
   * 只替换 origin（protocol + host），保留原有路径（如 /v1），
   * 解决中国版（minimaxi.com）和国际版（minimax.io）域名不一致的问题
   */
  modifyModels(models, credentials) {
    if (!credentials?.resourceUrl) return models;
    let targetOrigin;
    try { targetOrigin = new URL(credentials.resourceUrl).origin; } catch { return models; }
    return models.map(m => {
      if (m.provider !== "minimax" && m.provider !== "minimax-oauth") return m;
      try {
        const current = new URL(m.baseUrl);
        const target = new URL(targetOrigin);
        current.protocol = target.protocol;
        current.host = target.host;
        return { ...m, baseUrl: current.href.replace(/\/$/, "") };
      } catch { return m; }
    });
  },
};
